    ...
    <div className="min-h-screen bg-background font-[Poppins]">
      ...
      <nav className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-xl border-b border-border/60 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <Badge className="bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:bg-emerald-950/50 font-mono text-xs">Community</Badge>
          </div>
          <div className="flex items-center gap-4">
            <Link to="/docs">
              <Button variant="ghost" size="sm" className="text-sm text-muted-foreground hover:text-indigo-600 dark:text-indigo-400">Docs</Button>
            </Link>
            <Link to="/api-reference">
              <Button variant="ghost" size="sm" className="text-sm text-muted-foreground hover:text-indigo-600 dark:text-indigo-400">API</Button>
            </Link>
            <Link to="/login">
      ...
      <main className="max-w-5xl mx-auto px-6 sm:px-8 pt-28 pb-16">
        ...
        <div className="text-center mb-16">
          <div className="flex items-center justify-center gap-2 text-sm text-emerald-600 dark:text-emerald-400 font-medium mb-4">
          ...
          {COMMUNITY_LINKS.map((ch) => {
            ...
            return (
              <a key={ch.title} href={ch.href} target="_blank" rel="noopener noreferrer" className="group block">
                <div className="bg-card p-6 rounded-2xl border border-border shadow-sm hover:shadow-xl hover:border-indigo-200 dark:border-indigo-900/60 transition-all duration-300 h-full flex flex-col">
                  ...
                  <p className="text-sm text-muted-foreground flex-1 leading-relaxed">{ch.desc}</p>
                  <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 text-sm font-medium mt-4 group-hover:gap-3 transition-all">
        ...
        <section className="mb-16">
          <h2 className="text-2xl font-bold text-foreground mb-8 text-center">Get Involved</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { icon: HelpCircle, title: 'Ask & Answer', desc: 'Help others in discussions and earn community recognition', color: 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40' },
              { icon: Lightbulb, title: 'Share Ideas', desc: 'Propose features and vote on the roadmap priorities', color: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40' },
              { icon: Code2, title: 'Contribute Code', desc: 'Submit pull requests, fix bugs, or improve documentation', color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40' },
              { icon: Star, title: 'Star on GitHub', desc: 'Show your support and help us reach more teams', color: 'text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40' },
            ].map((item) => {
              const Icon = item.icon;
              return (
                <div key={item.title} className="bg-card p-5 rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow">
        ...
        <section className="mb-16">
          ...
          <div className="flex justify-center gap-6 flex-wrap">
            {CONTRIBUTORS.map((c) => (
              <div key={c.name} className="bg-card p-6 rounded-2xl border border-border shadow-sm text-center min-w-[160px] hover:shadow-md transition-shadow">
        ...
        <section className="mb-16">
          <h2 className="text-2xl font-bold text-foreground mb-8 text-center flex items-center justify-center gap-2">
            <HelpCircle className="w-6 h-6 text-indigo-600 dark:text-indigo-400" /> FAQ
          </h2>
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-violet-600 rounded-3xl p-8 sm:p-12 text-center text-white shadow-xl">
            ...
            <div className="flex gap-3 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-full bg-card/20 backdrop-blur text-white placeholder:text-indigo-200 border border-white/20 outline-none focus:ring-2 ring-white/30 text-sm"
              />
              <Button className="bg-card text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:bg-indigo-950/40 rounded-full px-6 font-semibold shadow-md">Subscribe</Button>
            </div>
          </div>
        </section>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-3xl p-8 sm:p-12 text-center shadow-xl">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <Link to="/login">
              <Button className="bg-card text-foreground hover:bg-muted rounded-full px-6 shadow-md">

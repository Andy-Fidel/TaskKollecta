...
import {
    ...
    ChevronRight, ChevronLeft, Check, Loader2, Sparkles, PartyPopper
} from 'lucide-react';
...
export default function OnboardingWizard() {
    ...
    const canProceed = () => {
        if (isInvitee) return true; // Invitees can always proceed
        switch (step) {
            ...
            <div className="w-full max-w-2xl">
                ...
                <div className="mb-8">
                  ...
                  ${i < step ? 'bg-green-500 text-white' :
                                        i === step ? 'bg-purple-500 text-white ring-4 ring-purple-500/30' :
                                            'bg-white/10 text-white/50'}
                `}>
                                    {i < step ? <Check className="w-5 h-5" /> : <s.icon className="w-5 h-5" />}
                                </div>
                                {i < STEPS.length - 1 && (
                                    <div className={`w-16 sm:w-24 h-1 mx-2 rounded ${i < step ? 'bg-green-500' : 'bg-white/10'}`} />
                                )}
                            </div>
                        ))}
                    </div>
                        ...
                        {isInvitee && step === 0 && (
                            <div className="space-y-6 animate-in fade-in duration-500">
                                <div className="text-center mb-8">
                                    <div className="inline-flex items-center justify-center w-20 h-20 bg-green-500/20 rounded-2xl mb-4">
                                ...
                                <div className="bg-white/10 rounded-xl p-6 text-center">
                                    <p className="text-white/80 mb-2">You've been added to:</p>
                        ...
                        {!isInvitee && step === 0 && (
                            <div className="space-y-6 animate-in fade-in duration-500">
                                ...
                                <div className="space-y-3">
                                    <Label className="text-white/80">How will you use TaskKollecta?</Label>
                                    {ROLES.map((role) => (
                                        <button
                                            key={role.id}
                                        ...
                                        >
                                            <div className={`p-2 rounded-lg ${formData.role === role.id ? 'bg-purple-500' : 'bg-white/10'}`}>
                            ...
                            <div className="space-y-6 animate-in fade-in duration-500">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <Label className="text-white/80">Workspace Name</Label>
                                        <Input
                                    ...
                                    {formData.inviteEmails.map((email, i) => (
                                        <div key={i} className="relative">
                                            ...
                                            <Input
                                                ...
                                                className="bg-white/10 border-white/20 text-white placeholder:text-white/40 h-12 pl-10"
                                            />
                                        </div>
                                    ))}
